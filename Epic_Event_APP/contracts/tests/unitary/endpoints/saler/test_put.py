...
class TestPutEndpoint:
    ...
    def test_should_update(self, create_contract, api_client, create_customer, saler_user):
    ...
    def test_should_not_put(self, create_contract, api_client, create_customer, saler_user):
        """test put with other saler user attached to the contract"""
        other_saler_user = User.objects.create(username='other_saler', password='motdepasse78')
        contract_object = create_contract
        payload = {
            'sales_contact_id': saler_user,
            'customer_id': create_customer.id,
