...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Permissions for the {supports_group.name} have been added successfully"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"Permissions for the {salers_group.name} have been added successfully"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"Permissions for the {gestions_group.name} have been added successfully"
            )
        )

    def get_saler_permissions(self) -> Permission:
        """Get and return the correct permission for salers team"""
        customer_content_type = ContentType.objects.get(
            app_label="customers", model="customer"
        )
        customer_permissions = Permission.objects.filter(
            content_type=customer_content_type
        ).exclude(name="Can delete customer")
        ...
        ).exclude(name="Can delete contract")
        event_content_type = ContentType.objects.get(app_label="events", model="event")
        event_permission = Permission.objects.filter(
            content_type=event_content_type
        ).exclude(name="Can delete event")

        permissions = chain(
            customer_permissions, contract_permissions, event_permission
        )

        return permissions

    def get_support_permissions(self) -> Permission:
        ...
        customer_permissions = Permission.objects.filter(
            content_type=customer_content_type
        ).filter(name="Can view customer")
        contract_content_type = ContentType.objects.get(
            app_label="contracts", model="contract"
        )
        contract_permissions = Permission.objects.filter(
            content_type=contract_content_type
        ).filter(name="Can view contract")
        event_content_type = ContentType.objects.get(app_label="events", model="event")
        event_permission = (
            Permission.objects.filter(content_type=event_content_type)
            .exclude(name="Can delete event")
