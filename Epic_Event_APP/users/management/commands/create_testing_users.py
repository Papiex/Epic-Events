...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if (
            User.objects.filter(first_name="gestion_user")
            ...
            if (
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully created Gestion, Saler, Support and Admin User, check the README for the users identifers"
                    )
                )
            else:
                self.stdout.write(
                    self.style.ERROR(
                        f"Somenthing went wrong with the command, please try again"
                    )
                )
