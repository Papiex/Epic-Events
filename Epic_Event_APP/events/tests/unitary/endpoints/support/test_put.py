...
class TestPutEndpoint:
    ...
    def test_should_update(self, create_contract, create_event, saler_user, api_client, support_user):
    ...
    def test_should_not_update(self, create_contract, create_event, api_client, support_user):
        """test with other support user attached to the event"""
        other_support_user = User.objects.create(username='other_support', password='motdepasse78', role='SUPPORT')
        event_object = create_event
        payload = {
            'support_contact_id': support_user.id,
            'event_statut': False,
