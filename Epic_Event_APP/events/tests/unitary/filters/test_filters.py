...
class TestFilters:
    ...
    def test_should_return_two_results_with_email(self, saler_user, api_client, payload_01, payload_02):
        ...
        response = api_client.get('/events/?customer_id__email=test@mail.fr')

        assert response.status_code == 200
        assert len(response.data) == 2


    def test_should_return_two_results_with_last_name(self, saler_user, api_client, payload_01, payload_02):
        ...
        refresh = RefreshToken.for_user(saler_user)
        api_client.credentials(HTTP_AUTHORIZATION=f'Bearer {refresh.access_token}')
        response = api_client.get('/events/?customer_id__last_name=test')

        assert response.status_code == 200
        assert len(response.data) == 2


    def test_should_return_two_results_with_date(self, saler_user, api_client, payload_01, payload_02):
